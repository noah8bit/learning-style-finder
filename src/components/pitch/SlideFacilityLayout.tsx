...
export default function SlideFacilityLayout({ active }: { active: boolean }) {
  const zones = [
    { name: "Main Training Floor", size: "15,000 sq ft", icon: "🏋", color: "border-primary bg-primary/10" },
    { name: "Track & Field Complex", size: "8,000 sq ft", icon: "🏃", color: "border-accent bg-accent/10" },
    { name: "Cross-Training Arena", size: "6,000 sq ft", icon: "⚡", color: "border-primary bg-primary/10" },
    { name: "Indoor Courts", size: "8,000 sq ft", icon: "🏀", color: "border-accent bg-accent/10" },
    { name: "Swimming Center", size: "5,000 sq ft", icon: "🏊", color: "border-primary bg-primary/10" },
    { name: "Film & Stunt Wing", size: "6,000 sq ft", icon: "🎬", color: "border-accent bg-accent/10" },
    { name: "Recovery Wing", size: "4,000 sq ft", icon: "❄️", color: "border-primary bg-primary/10" },
    { name: "Medical Suite", size: "2,500 sq ft", icon: "🏥", color: "border-accent bg-accent/10" },
    { name: "Seminar & Events", size: "2,500 sq ft", icon: "🎓", color: "border-primary bg-primary/10" },
    { name: "Golf & Specialty", size: "3,000 sq ft", icon: "⛳", color: "border-accent bg-accent/10" },
    { name: "Outdoor Fields", size: "Adj. Acreage", icon: "🌳", color: "border-green-500 bg-green-500/10" },
  ];
  ...
  return (
    <ScaledSlide>
      <div className="w-full h-full bg-background flex">
        <div className="w-[55%] flex flex-col justify-center px-[70px]">
          {active && (
            <>
              ...
              <div className="animate-fade-up stagger-3 grid grid-cols-2 gap-3 mb-6">
                {stats.map((s, i) => (
                  <div key={i} className="bg-secondary border border-border rounded-xl p-4 text-center">
                    ...
                    <div className="text-muted-foreground text-[16px]">{s.label}</div>
                  </div>
                ))}
              </div>
                ...
                {zones.map((z, i) => (
                  <div key={i} className={`${z.color} border rounded-lg p-3 flex items-center gap-4`}>
                    ...
                    <div className="flex-1 flex justify-between items-baseline">
                      <h3 className="text-foreground text-[18px] font-bold">{z.name}</h3>
                      <span className="text-primary text-[16px] font-semibold shrink-0 ml-2">{z.size}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="w-[45%] h-full flex items-center justify-center px-[30px]">
          {active && (
            <div className="animate-fade-up stagger-4 relative w-full h-[85%] bg-secondary/50 border border-border rounded-2xl overflow-hidden">
              {[
                ...
                { name: "Seminar", icon: "🎓", x: "3%", y: "72%", w: "22%", h: "14%", color: "border-primary bg-primary/10" },
                { name: "Golf", icon: "⛳", x: "27%", y: "72%", w: "22%", h: "14%", color: "border-accent bg-accent/10" },
                { name: "Outdoor", icon: "🌳", x: "51%", y: "72%", w: "46%", h: "25%", color: "border-green-500 bg-green-500/10" },
              ].map((z, i) => (
                <div
                  key={i}
                ...
                >
                  <span className="text-[22px] mb-0.5">{z.icon}</span>
                  <span className="text-foreground text-[14px] font-bold leading-tight">{z.name}</span>
                </div>
              ))}
            </div>
          )}
